    ...
    <div className="min-h-screen pb-24 bg-background" dir={dir} data-testid="donations-page">
      <div className="sticky top-0 z-50 bg-background/95 backdrop-blur-xl border-b border-border/20 px-4 h-14 flex items-center justify-between">
        ...
        {user && <button onClick={() => setShowCreate(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-full bg-primary text-primary-foreground text-xs font-bold active:scale-95">
          <Plus className="h-3.5 w-3.5" />طلب مساعدة
        </button>}
      </div>
      ...
      <div className="mx-4 mt-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 p-4">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p className="text-sm font-bold text-foreground">تنبيه مهم</p>
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">نحن فقط وسيلة لإيصالكم لبعض. لا نتحمل أي مسؤولية عن المعاملات بين الأطراف. فقط ادعو لوالدَيّ بالرحمة والمغفرة.</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 py-4">
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-base font-bold text-muted-foreground/50">لا توجد طلبات حالياً</p>
            <p className="text-xs text-muted-foreground/30 mt-1">كن أول من ينشر طلب مساعدة</p>
          </div>
        ) : (
          <div className="space-y-3">
            {requests.map(r => (
              <div key={r.id} className="rounded-2xl bg-card border border-border/30 p-4">
                ...
                {r.amount_needed && <p className="text-sm font-bold text-primary mt-2">المبلغ المطلوب: {r.amount_needed}</p>}
                <div className="mt-3 pt-3 border-t border-border/20">
                  <a href={`mailto:${r.contact_info}`} className="flex items-center gap-2 text-xs text-primary font-bold">
                    <MessageSquare className="h-3.5 w-3.5" />تواصل: {r.contact_info}
                  </a>
                </div>
              </div>
            ))}
    ...
    <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
      ...
      <div className="bg-card rounded-t-3xl max-h-[85vh] flex flex-col border-t border-primary/20">
        <div className="flex items-center justify-between p-4 border-b border-border/20">
          ...
          <h3 className="text-sm font-bold">طلب مساعدة</h3>
        ...
        <div className="p-4 space-y-3 overflow-y-auto">
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="عنوان الطلب *" dir="auto"
            className="w-full bg-muted/40 rounded-xl px-4 py-3 text-sm outline-none text-foreground border border-border/30" />
          <textarea value={desc} onChange={e => setDesc(e.target.value)} placeholder="وصف الحالة *" dir="auto"
          ...
          <input value={contact} onChange={e => setContact(e.target.value)} placeholder="معلومات التواصل (بريد أو رقم) *" dir="auto"
            className="w-full bg-muted/40 rounded-xl px-4 py-3 text-sm outline-none text-foreground border border-border/30" />
          <input value={amount} onChange={e => setAmount(e.target.value)} placeholder="المبلغ المطلوب (اختياري)" dir="auto"
            className="w-full bg-muted/40 rounded-xl px-4 py-3 text-sm outline-none text-foreground border border-border/30" />
        </div>
      </div>
    </motion.div>
