  ...
  const renderQuran = () => {
    ...
    if(selectedSurah) return(<div className="space-y-4 pb-8">
      ...
      {selectedSurah.ayahs?.map((a:any,i:number)=>(
        <div key={i} className="w-full rounded-2xl bg-card/60 border border-border/30 hover:border-emerald-400/30 transition-all overflow-hidden">
          {/* V2026: Use GlobalQuranVerse for each ayah */}
          <GlobalQuranVerse
            surahId={selectedSurah.number || selectedSurah.id}
            ayahId={a.number || (i + 1)}
            compact={false}
