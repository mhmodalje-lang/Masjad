...
export default function ScenarioGame({ scenario, options, correctIndex, explanation, xp, onComplete }: ScenarioProps) {
  ...
  const handleSelect = (idx: number) => {
    ...
    setRevealed(true);
    setTimeout(() => {
      onComplete(idx === correctIndex, idx === correctIndex ? xp : 0);
    }, 4000);
  };
    ...
    <div className="flex flex-col items-center px-4 py-4 w-full max-w-lg mx-auto">
      ...
      >
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-xs font-bold text-violet-400 uppercase tracking-wider">{t('digitalShieldScenario') || 'Digital Shield Scenario'}</span>
      ...
      <div className="w-full space-y-3">
        {options.map((opt, idx) => {
          ...
          if (revealed) {
            if (idx === correctIndex) {
              bgClass = 'bg-emerald-500/20 border-emerald-400/40 ring-2 ring-emerald-400/30';
              textClass = 'text-emerald-300';
            } else if (idx === selected) {
              bgClass = 'bg-red-500/20 border-red-400/40 ring-2 ring-red-400/30';
              textClass = 'text-red-300';
            } else {
              bgClass = 'bg-gray-800/30 border-gray-700/30';
        ...
        {revealed && (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-amber-400">{t('explanation') || 'Did you know?'}</span>
            </div>
      ...
      <AnimatePresence>
        {revealed && (
          ...
          >
            {selected === correctIndex ? `🛡️ ${t('goodChoice') || 'Smart Choice!'} +${xp} XP` : `⚠️ ${t('beCareful') || 'Be more careful next time!'}`}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
