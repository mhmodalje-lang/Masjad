    ...
    <div className="flex flex-col items-center px-4 py-6 w-full max-w-lg mx-auto">
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">{t('knowledgeChallenge') || 'Knowledge Challenge'}</span>
        ...
        {options.map((opt, idx) => {
          ...
          if (revealed) {
            if (isCorrect) {
              ...
              borderClass = 'border-emerald-300 ring-2 ring-emerald-400/50';
            } else if (isSelected && !isCorrect) {
              bgClass = 'bg-red-500/80';
              borderClass = 'border-red-300 ring-2 ring-red-400/50';
            } else {
              bgClass = 'bg-gray-600/40';
          ...
          return (
            <motion.button
              ...
              disabled={revealed}
              className={`relative w-full py-4 px-5 rounded-2xl border-2 ${bgClass} ${borderClass} ${textClass} font-bold text-base text-start transition-all shadow-md active:shadow-sm`}
            >
      ...
      <AnimatePresence>
        {revealed && (
          ...
          >
            {selected === correctIndex ? `🎉 ${t('correct') || 'Correct!'} +${xp} XP` : `😔 ${t('tryAgain') || 'Not quite!'}`}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
